...
function loadFiles(): { id: string; children: FileEntry[] }[] {
...
const filesLoader: Loader = {
  ...
  load: ({ store, watcher, logger }) => {
    store.clear();
    for (const entry of loadFiles()) {
      store.set({ id: entry.id, data: { children: entry.children } });
    }

    // Dev mode: watch public directory for changes
    watcher?.on('all', (event, changedPath) => {
      if (changedPath.startsWith(publicDir)) {
        logger.info(`File ${event}: ${path.relative(publicDir, changedPath)}`);
        store.clear();
        for (const entry of loadFiles()) {
          store.set({ id: entry.id, data: { children: entry.children } });
        }
